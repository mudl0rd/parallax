 ...
 * Implementation
 */
jit_pointer_t jit_memcpy(jit_pointer_t dst, const void *src, jit_word_t size)
{
	if (size)
...
jit_pointer_t jit_memmove(jit_pointer_t dst, const void *src, jit_word_t size)
{
	if (size)
...
void jit_set_memory_functions(jit_alloc_func_ptr alloc_ptr, jit_realloc_func_ptr realloc_ptr,
...
void jit_get_memory_functions(jit_alloc_func_ptr *alloc_ptr, jit_realloc_func_ptr *realloc_ptr,
                              jit_free_func_ptr *free_ptr)
{
...
static void *jit_default_alloc_func(size_t size)
{
	return (malloc(size));
}

static void *jit_default_realloc_func(void *ptr, size_t size)
{
