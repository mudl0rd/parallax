...
static void _split_branches(jit_state_t *_jit);
...
void _jit_dataset(jit_state_t *_jit)
{
	...
	if (!_jit->user_data)
	{
		...
		_jit->data.length = (_jitc->data.offset +
		                     ...
		                     _jitc->note.size + 4095) &
		                    -4096;
#if defined(__sgi)
		mmap_fd = open("/dev/zero", O_RDWR);
#endif
		_jit->data.ptr = mmap(NULL, _jit->data.length, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, mmap_fd, 0);
...
jit_pointer_t _jit_emit(jit_state_t *_jit)
{
...
#endif
		_jit->code.ptr =
		    mmap(NULL, _jit->code.length, PROT_EXEC | PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, mmap_fd, 0);
...
#if HAVE_MREMAP
#if __NetBSD__
			_jit->code.ptr = mremap(_jit->code.ptr, _jit->code.length, _jit->code.ptr, length, 0);
#else
			_jit->code.ptr = mremap(_jit->code.ptr, _jit->code.length, length, MREMAP_MAYMOVE, NULL);
#endif
#else
			_jit->code.ptr = mmap(NULL, length, PROT_EXEC | PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, mmap_fd, 0);
#endif
	...
	{
#ifdef _WIN32
		result = _mprotect(_jit->code.ptr, _jit->code.length, PROT_READ | PROT_EXEC);
#else
	...
	for (node = block->label->next; node; node = node->next)
	{
		 ...
		 * handle registers that are both, argument and result */
			...
			if (value & jit_cc_a0_reg)
			{
				...
				if (value & jit_cc_a0_rlh)
				{
					if (!(node->u.q.l & jit_regno_patch) && jit_regset_tstbit(&block->regmask, node->u.q.l))
					{
						jit_regset_clrbit(&block->regmask, node->u.q.l);
						if (live)
							jit_regset_setbit(&block->reglive, node->u.q.l);
					}
					if (!(node->u.q.h & jit_regno_patch) && jit_regset_tstbit(&block->regmask, node->u.q.h))
					{
				...
				{
					if (!(node->u.w & jit_regno_patch) && jit_regset_tstbit(&block->regmask, node->u.w))
					{
...
static void _split_branches(jit_state_t *_jit)
{
	...
	for (node = _jitc->head; node; node = next)
	{
		if ((next = node->next))
		{
			if (next->code == jit_code_label || next->code == jit_code_prolog || next->code == jit_code_epilog)
				continue;
